...
<body>
	<%
		...
		if (userID == null) {
			...
			script.println("alert('[update]Log in first')");
		...
		if (bbsID == 0) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('[update]Nonvalidated post')");
		...
		if (!userID.equals(bbs.getUserID())) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('[update]Have no permission')");
			script.println("location.href = 'bbs.jsp'");
			script.println("</script>");
		}
 	%>
		...
		<div class="row">
			<form method="post" action="updateAction.jsp?bbsID=<%= bbsID %>">
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
					<thead>
						<tr>
							<th colspan="2" style="background-color: #eeeeee; text-align: center;">Correct Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
				...
				<input type="submit" class="btn btn-primary pull-right" value="Correct">
			</form>
		</div>
	</div>
